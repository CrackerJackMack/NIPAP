...
class LdapAuth(BaseAuth):
    ...
    def __init__(self, name, username, password, authoritative_source, auth_options=None):
        ...
        if self._cfg.has_option(base_auth_backend, 'cacert'):
            self._ldap_cacert = self._cfg.get(base_auth_backend, 'cacert')
        ...
        if self._ldap_cacert:
            # ssl settings for most versions are global instead of per connection.
            ldap.set_option(ldap.OPT_X_TLS_CACERTFILE, self._ldap_cacert)
